...
// WithSessionID / WithSystemBytes return a new *ControlMessage; no method mutates the receiver.
...
// ToBytes serializes the control message to its on-wire representation: a 4-byte length prefix (always 0x00_00_00_0A = 10) followed by the 10-byte header.
//
// The returned slice is always exactly 14 bytes.
...
//
// For Linktest/Select/Deselect/Reject req/rsp pairs this is the transaction correlation key. Equivalent to FromSystemBytes(msg.SystemBytes()).
...
// WithSessionID returns a new *ControlMessage identical to msg except that the session ID in header bytes 0–1 is replaced by id.
...
// WithSystemBytes returns a new *ControlMessage identical to msg except that header bytes 6–9 are replaced by b.
...
const (
	...
	// SelectStatusAlreadyUsed indicates that the TCP/IP port is exhausted; another connection is already established.
	...
	// DeselectStatusNotEstablished indicates that HSMS communication has not yet been established with a Select, or has already been ended with a previous Deselect.
	DeselectStatusNotEstablished = 1
	// DeselectStatusBusy indicates that the session is still in use and cannot yet be relinquished gracefully.
...
// req must be non-nil.
//
// selectStatus 0 = communication successfully established; 1 = already active; 2 = not ready; 3 = TCP/IP port exhausted;
...
// req must be non-nil.
//
// deselectStatus 0 = connection successfully ended; 1 = communication not yet established; 2 = busy, cannot yet relinquish;
...
// NewLinktestRsp creates an HSMS Linktest.rsp control message from a Linktest.req. req must be non-nil.
