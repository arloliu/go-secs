...
type TransportRuntime interface {
	...
	//
	// The transport transfers ownership of frame; the core must not be called with the same buffer again.
	...
	// WriteMessage performs a synchronous framed write of msg over the current epoch's TCP connection.
	//
	// It uses a core-owned writev.
	// Used for W-bit primary data messages and synchronous control sends.
	//
	// Returns the sent message (for System-Bytes accounting) and any write error.
	WriteMessage(ctx context.Context, msg Message) (Message, error)

	// WriteMessageNoReply performs a synchronous framed write of msg WITHOUT registering a reply expectation.
	//
	...
	//
	// Returns the write error, or nil once the frame is on the wire.
	...
	// NextSystemBytes returns the next unique System Bytes value for an outbound control request.
	//
