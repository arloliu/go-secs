...
type SECS2Endpoint interface {
	...
	//
	// When replyExpected is true, it waits for the matching secondary reply or a
	// protocol/context timeout.
	//
	...
	//
	// It preserves the message's System Bytes, W-bit, session ID, and stream/function, and
	// returns once the frame is on the wire.
	//
	// Unlike SendDataMessage it does NOT register a reply expectation: any secondary the
	// peer sends misses the reply registry and is delivered to the registered
	// DataMessageHandlers instead of being returned here.
	//
	// Use this when YOU own reply correlation — a router or proxy bridging transports that
	// matches a reply to an upstream request by System Bytes.
	//
	// Ordinary request/reply code should use SendDataMessage.
	//
	// A W-bit-set msg keeps its W-bit on the wire (the peer is still asked to reply); only
	// the LOCAL reply-wait is skipped.
	//
	// Returns ErrNilMessage if msg is nil.
	//
	// The caller owns System Bytes: they are sent verbatim and are NOT drawn from the
	// connection's monotonic generator, so they may collide with an in-flight
	// SendDataMessage/SendSECS2Message transaction OR another forwarded message on the
	// same connection.
	//
	// On a collision the peer's reply is routed to whichever waiter/handler the registry
	// resolves — reply theft.
	//
	// Keep forwarded System Bytes disjoint from the library-generated space (which
	// starts at 1 and increments).
	//
	// The message's session ID is sent verbatim on HSMS-SS.
	//
	// If the connection enables WithSessionIDValidation, a forwarded session ID that
	// differs from the connection's makes the peer's reply fail inbound validation and
	// be dropped (and answered with an S9F1), so it never reaches your handlers —
	// forward with the connection's own session ID, or leave validation disabled.
	//
	// On SECS-I the block-header device ID is always the configured device ID regardless
	// of msg's session ID (only System Bytes / stream / function / W-bit go on the wire
	// verbatim).
	...
	//
	// The message is enqueued on the per-generation async send channel instead of writing on
	...
	//
	// The reply reuses the primary message's System Bytes verbatim (E37 §8.2.6.9) and carries
	// no W-bit.
...
type Connection interface {
	...
	//
	// For an active connection under OpenBackground, a peer that is not yet reachable at
	// Open time is not an error: Open returns nil and the connection retries the initial
	// connect in the background (see the (*connection).Open doc for the exact
	// NotConnectedState-only scoping).
	Open(ctx context.Context, mode OpenMode) error

	// Close tears down the connection and all per-generation resources.
	//
	// Close is ctx-free and always completes teardown (internally bounded by the configured
	// close timeout).
	//
