...
// DataMessageCodec wraps a *DataMessage to satisfy encoding.BinaryMarshaler and encoding.BinaryUnmarshaler, for storage layers whose contract requires those standard-library interfaces (v1's *DataMessage satisfied them directly;
// v2's DataMessage stays fully immutable — no method mutates it in place — so the codec is a separate wrapper rather than new methods on DataMessage itself).
//
// UnmarshalBinary never mutates an existing Message: it decodes data into a fresh *DataMessage
// and repoints the Message field, exactly like assigning the result of DecodeHSMSMessage yourself.
...
// Codec wraps msg in a *DataMessageCodec, for storing msg in a slot whose contract requires encoding.BinaryMarshaler / encoding.BinaryUnmarshaler (see DataMessageCodec) —
// e.g. a transport-agnostic message envelope in application code.
//
// Returns *DataMessageCodec, not a value: UnmarshalBinary has a pointer receiver (it mutates c.Message in place),
// and Go does not promote pointer-receiver methods into a value's method set for interface satisfaction —
// a DataMessageCodec value assigned into an encoding.BinaryUnmarshaler-typed slot would silently fail to satisfy it. msg may be nil;
// a subsequent MarshalBinary on the result then returns ErrNilMessage exactly as calling it on a zero-value DataMessageCodec already does.
func (msg *DataMessage) Codec() *DataMessageCodec {
	return &DataMessageCodec{Message: msg}
}

// MarshalBinary serializes the wrapped message via its ToBytes method.
//
// Returns an error if Message is nil.
...
// UnmarshalBinary decodes data — a length-prefixed HSMS frame, see DecodeHSMSMessage — and stores the result in c.Message.
//
// Returns an error if data fails to decode, or decodes to a control message rather than a data message.
...
// DecodeErr returns the wrapped message's deferred decode error, or ErrNilMessage if Message is nil.
...
// ToDataMessage returns the wrapped message and true, or (nil, false) if Message is nil. This is the zero-safe unwrap probe.
