...
// passive connection accepts + auto-commits to Selected and processes an inbound block over an
// in-memory pipe-backed fake net.Listener.
//
// The file is in package secs1 (not secs1_test) so it can read the unexported cfg.listen field,
// construct the unexported *transport, and reuse the peer-scripting helpers (peerWrite, peerReadN,
// makeTestBlock, the enq/eot/ack control bytes) from the sibling test files. Mirrors
// transport_dialer_test.go's four-test shape for the active-dial seam.
...
// TestWithListener_PipeAcceptance drives a REAL passive SECS-I connection over an in-memory fake
// net.Listener injected via WithListener: the transport's Accept call is satisfied by one end of a
// fresh net.Pipe, the connection auto-commits to Selected as soon as the peer is adopted (SECS-I
// has no Select handshake — a live line IS the selected session), and a scripted peer then drives
// the SECS-I ENQ/EOT/block/ACK line protocol (SEMI E4 §7.8.2) to prove the accepted socket's line
// engine is actually live, not just that TCPUp/CommitSelected fired. Close tears the connection
// down cleanly. Mirrors TestWithDialer_PipeAcceptance for the active side.
func TestWithListener_PipeAcceptance(t *testing.T) {
	...
	cfg, err := NewConfig("127.0.0.1", 5000, WithPassive(), WithDeviceID(0x1234), WithListener(listen))
	require.NoError(t, err)
	...
	// Drive the scripted peer side: dial (via the fake listener's pipe generation) then run the full
	// ENQ/EOT/block/ACK handshake once the accepted line engine starts polling. peerWrite/peerReadN
	// and makeTestBlock/enq/eot/ack are the shared SECS-I line-protocol test helpers already used by
	// the sibling *_test.go files in this package (e.g. line_test.go, transport_lifecycle_test.go).
	peer := fakeLn.dialFresh()
	golden := makeTestBlock(t, []byte("listener"))
	...
	require.NoError(t, conn.Open(ctx, hsms.OpenWaitSelected),
		"passive connection must accept over the fake listener and auto-commit to Selected")
	require.Equal(t, hsms.SelectedState, conn.State())

	select {
	case got := <-ackCh:
		require.Equal(t, []byte{ack}, got, "the accepted line engine must ACK a well-formed inbound block")
	case <-time.After(5 * time.Second):
		t.Fatal("passive engine never ACKed the inbound block over the fake listener")
	}
