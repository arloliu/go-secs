...
//
// It embeds [hsms.ConnectionConfig] so the shared core knobs it reuses (T1/T2/T3/T4/T5 timers, session/device wiring, logger, close/queue policy) are promoted directly onto Config
// and read via the same accessors.
// SECS-I-specific settings — the TCP endpoint, active/passive role, keep-alive, the RTY retry limit, and the equipment/host role
// and device ID — are separate unexported fields set through [Option] values.
//
// Note two SECS-I invariants Config enforces at construction (see [NewConfig]):
//   - The embedded core write timeout is forced to 0 so a core write deadline can never preempt a SECS-I line transaction,
//     which is bounded instead by T2 × (RetryLimit+1).
//   - The core auto-linktest stays inert: secs1 never exposes a linktest option nor sets an interval,
//     and the embedded default interval is 0 (disabled), so no linktest goroutine arms.
type Config struct {
...
// so callers can keep writing secs1.DialFunc without importing hsms for this alone.
...
//
// The application is all-or-nothing: every option is validated on a scratch copy; if any option returns an error the original is discarded
// and the joined set of errors is returned.
//
// The embedded [hsms.ConnectionConfig] is initialised to the core defaults (T3=45 s reply timeout, sessionID=0xFFFF, logger set).
// The SECS-I timers default to the SEMI E4 recommendations: T1=500 ms, T2=10 s, T4=45 s, RetryLimit=3.
// The default role is active (dials outbound) and host (isEquip=false); pass [WithPassive] / [WithEquipment] / [WithHost] to change them.
//
// As its LAST construction step — after all user options have applied — NewConfig FORCES the embedded writeTimeout to 0.
// SECS-I bounds each on-wire block transaction with its own T2 × (RetryLimit+1) budget; a non-zero core write deadline could preempt
// that budget mid-transaction and desync the line, so the override is applied last
// and a user cannot re-arm it (e.g. via WithConnectionOption(hsms.WithWriteTimeout(...))).
//
// Also as a LAST step, NewConfig forces the core session ID to the configured SECS-I device ID (see [WithDeviceID]).
// The device ID IS the effective session identity, so the connection and its outbound messages report it through SessionID();
// a user cannot override it via WithConnectionOption(hsms.WithSessionID(...)).
...
//
// It does NOT re-apply the writeTimeout=0 override, so callers adjusting settings after construction must not re-arm writeTimeout through it.
...
//
// The exponential backoff between successive connect attempts after a TCP disconnect ramps up to at most this duration (see hsms.WithReconnectBackoff for the growth curve).
//
// SEMI E4 does not define a T5 timer; go-secs reuses the shared hsms T5 slot as the reconnect-cadence knob for BOTH transports,
// so this exists for parity with WithT1/WithT2/WithT4.
// Prior to this option, the only way to tune SECS-I's reconnect cadence was the WithConnectionOption(hsms.WithT5(...)) wrapper form,
// which left connections on the default value if omitted.
//
// The duration must be > 0.
...
// It also enables the shared hsms.WithAutoS9F9 behavior: a synchronous data-message send whose reply times out (T3) automatically notifies the peer with an S9F9 (SEMI E5 §10.13) before the timeout error is returned.
//
// The default role is host; use this option to override it. Equipment and host are mutually exclusive —
// whichever option appears last in a [NewConfig] call wins.
...
//
// Equipment and host are mutually exclusive — whichever option appears last in a [NewConfig] call wins (see WithEquipment).
...
//
// This allows callers to tune the shared core knobs that SECS-I reuses, without a separate re-export per option.
//
...
//   - [hsms.WithT5] — reconnect backoff ceiling; the exponential backoff between successive connect attempts after a TCP disconnect ramps up to at most this duration.
//   - [hsms.WithSenderQueueSize] — maximum number of messages that may be queued ahead of the line engine before SendDataMessage blocks.
//   - [hsms.WithCloseTimeout] — maximum time [hsms.Connection.Close] waits for a clean teardown before forcing the transport down.
//   - [hsms.WithLogger] — replaces the default no-op logger.
//
// [secs1.WithT1]/[secs1.WithT2]/[secs1.WithT4]/[secs1.WithT5] remain the preferred spelling for build-time config;
// this wrapper form exists so the SAME options also work with [hsms.Connection.UpdateConfigOptions] at runtime (SECS-I's [Connection] exposes only [hsms.ConnOption] there, not [secs1.Option]).
//
// Knobs that are inert or overridden for SECS-I (do not use):
//   - [hsms.WithWriteTimeout] — [NewConfig] forces writeTimeout to 0 as its last construction step;
//     any value set here is overridden.
//   - [hsms.WithSessionID] — [NewConfig] forces the session ID to the SECS-I device ID (see [WithDeviceID]) as its last construction step;
//     any value set here is overridden.
//   - [hsms.WithLinktestInterval], [hsms.WithLinktestFailThreshold] — SECS-I never arms an auto-linktest;
//     both settings are ignored.
//   - [hsms.WithT6], [hsms.WithT7], [hsms.WithT8] — HSMS-SS-specific timers with no counterpart in SEMI E4;
//     setting them has no effect on the SECS-I transport.
//
// Example:
//
