...
//
// These represent the wire-framing layer beneath the shared hsms data-message counters (see hsms.ConnectionMetrics, reached via Connection's embedded hsms.Connection.Metrics()).
//
...
//
// A retry is triggered by a NAK, a T2 timeout, or a failed receive during a slave-contention yield (SEMI E4 §7.8.2).
...
//
// Unlike BlockRetryCount (routine, expected under a noisy line), this represents a link-failure signal worth alerting on (SEMI E4 §7.8.6).
...
//
// This aggregates length errors, checksum/parse errors, T1 timeouts reading the body, or T2 timeouts waiting for the length byte.
...
// ContentionYieldCount returns the total number of times this connection, acting as slave, yielded line control to a contending master (SEMI E4 §7.8.2.1) —
// a pure half-duplex contention signal.
...
// PartialTimeoutCount returns the total number of stale partial multi-block messages discarded after the T4 inter-block deadline elapsed (SEMI E4 §9.4.3).
...
//
// An abort occurs if blocks carry the wrong block number or a mismatched block-invariant header (SEMI E4 §9.4.4.2).
...
//
// Dropping occurs if the block is neither block number 1 nor a lone block 0 with the E-bit set (SEMI E4 §9.4.4.2).
