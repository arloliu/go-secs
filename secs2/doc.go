...
// Individual-element accessors return values by value, never by pointer or reference to internal state.
// AppendBinaryTo appends raw bytes into a caller-supplied buffer and never returns an internal slice.
//
// # Accessor families
//
// Three families of accessors cover different performance and convenience trade-offs:
//
//   - Copy accessors (ToList, ToBinary, ToBoolean, ToInt, ToUint, ToFloat, ToASCII, ToJIS8, ToLocalizedStr, ToLocalizedStrHeader) return a fresh, caller-owned copy of the data.
//
//   - Zero-copy iterators (Items, Bools, Ints, Uints, Floats) return an iter.Seq that yields values directly from internal storage;
//     the sequence is safe to call concurrently.
//     Iterating allocates nothing when called on the concrete item type; calling through the Item interface costs a few allocations from interface dispatch (still cheaper than copying a large slice via the copy accessors).
//
...
// No allocation occurs when dst already has sufficient capacity.
//
// ToBytes() is a convenience wrapper equivalent to AppendTo(make([]byte, 0, EncodedLen())).
//
// EncodedLen() returns the exact total byte count, enabling callers to pre-size a buffer before calling AppendTo to avoid any allocation during encoding.
//
// # Construction
//
...
//
// Decode copies the input bytes (the caller may reuse or free the buffer immediately after the call returns).
...
// Binary, ASCII, JIS-8, and localized-string items then alias the buffer directly with no further copy; numeric
// and boolean items always build a typed value slice regardless.
package secs2
