...
An oversized message therefore never contends the write mutex at all — the rejection is pure CPU work
(summing the body's already-materialized `Buffers()`) with no lock acquisition, no conn access, and
no interaction with the I1 stale-epoch guard or the B2 Selected re-check that live inside the locked
section.
