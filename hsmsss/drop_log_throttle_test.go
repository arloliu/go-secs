...
// the drop gate before any TCP I/O.
func TestDropLog_ThrottledUnderFlood(t *testing.T) {
	require := require.New(t)
	ctx := t.Context()

	cl := newWarnCountLogger()
	cfg, err := NewConnectionConfig("localhost", 6666, WithHostRole(), WithLogger(cl))
	require.NoError(err)
	...
	for range drops {
		...
		require.ErrorIs(session.SendMessageAsync(m), hsms.ErrNotSelectedState)
	}
