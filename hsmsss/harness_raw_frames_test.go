...
// single-session control-frame convention (E37.1 §8.1).
...
// buildRejectFrame builds a fully-framed HSMS Reject.req (SType 7, E37 §7.10 / §8.3.20).
//
...
func buildRejectFrame(rejectedSTypeOrPType byte, reason byte, sb [4]byte) []byte {
	h := make([]byte, 10)
	binary.BigEndian.PutUint16(h[0:2], 0xFFFF) // control-frame session ID (E37.1 §8.1)
...
// a real Select.req header. header[4] (PType) is 0 and the session ID is 0xFFFF (E37.1 §8.1).
